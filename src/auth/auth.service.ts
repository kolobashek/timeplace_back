...
export class AuthService {
    ...
    async createEmailToken(email: string): Promise<boolean> {
        ...
        if (
            emailVerification &&
            (new Date().getTime() - emailVerification.timestamp.getTime()) /
                60000 <
                1
        ) {
            // < ** minutes
    ...
    async sendEmailForgotPassword(email: string): Promise<boolean> {
        ...
        if (tokenModel && tokenModel.newPasswordToken) {
          ...
          <a href=${process.env.APP_URL}:${process.env.APP_PORT}/auth/email/reset-password/${tokenModel.newPasswordToken}>Click here</a>`, // html body
            }

            var sended = await new Promise<boolean>(async function(
                resolve,
                reject
            ) {
                return await transporter.sendMail(
